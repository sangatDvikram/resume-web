...
async function validate(): Promise<void> {
  ...
  try {
    ...
    const languages    = await skillRepo.count({ where: { category: SkillCategory.language } });
    const frameworks   = await skillRepo.count({ where: { category: SkillCategory.framework } });
    const databases    = await skillRepo.count({ where: { category: SkillCategory.database } });
    const tools        = await skillRepo.count({ where: { category: SkillCategory.tool } });
    results.push(check('≥ 15 skill rows',    totalSkills  >= 15, `${totalSkills} rows`));
    results.push(check('language category',  languages    >  0,  `${languages} rows`));
    results.push(check('framework category', frameworks   >  0,  `${frameworks} rows`));
    results.push(check('database category',  databases    >  0,  `${databases} rows`));
    ...
    results.push(check('1 isCurrent=true entry', !!currentExp, currentExp?.company ?? 'none'));
    ...
    results.push(check('2 patent rows', patentCount === 2, `${patentCount} rows`));
    ...
    const publishedCount = await projectRepo.count({ where: { published: true } });
    const featuredCount  = await projectRepo.count({ where: { featured: true } });
    results.push(check('3 project rows',        projectCount  === 3, `${projectCount} rows`));
    results.push(check('all projects published', publishedCount === 3, `${publishedCount} published`));
    results.push(check('≥ 1 featured project',   featuredCount  >= 1, `${featuredCount} featured`));
    ...
    const projectWithSkills = await projectRepo
      ...
      .where('s.id IS NOT NULL')
      .getCount();
    results.push(check('projects have skills via join table', projectWithSkills > 0, `${projectWithSkills} with skills`));
    ...
    const photosWithAlbum = await photoRepo
      ...
      .getCount();
    results.push(check('all photos in general album', photosWithAlbum === 6, `${photosWithAlbum} linked`));
  ...
  console.log(`\n${'─'.repeat(60)}`);
  console.log(`Validation complete: ${results.length - failed.length}/${results.length} checks passed.`);
