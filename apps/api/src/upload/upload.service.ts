...
const VIDEO_SIGNATURES: Array<{ mime: string; bytes: number[]; offset: number }> = [
...
function detectVideoMime(buf: Buffer): string | null {
  ...
  return matchSig(buf, VIDEO_SIGNATURES.slice(1)) ?? matchSig(buf, VIDEO_SIGNATURES.slice(0, 1));
}
    ...
    return new Promise<PhotoUploadResult>((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        ...
        (error, result) => {
          ...
          const thumbUrl = originalUrl.replace('/upload/', '/upload/c_fill,w_400,f_auto,q_auto/');
          const lqipUrl  = originalUrl.replace('/upload/', '/upload/e_blur:2000,q_1,f_auto/');

          // Map EXIF from Cloudinary response (keys follow Exif standard names)
          const meta: Record<string, string> = (result as any).image_metadata ?? {};
